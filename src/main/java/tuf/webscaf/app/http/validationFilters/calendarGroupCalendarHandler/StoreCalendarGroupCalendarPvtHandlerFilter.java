...
public class StoreCalendarGroupCalendarPvtHandlerFilter implements HandlerFilterFunction<ServerResponse, ServerResponse> {
    @Override
    public Mono<ServerResponse> filter(ServerRequest request, HandlerFunction<ServerResponse> next) {
        ...
        return request.formData()
                .flatMap(value -> {
                    ...
                    List<String> listOfCalendar = value.get("calendarUUID");
                    boolean notValid = false;
                    if (value.containsKey("calendarUUID")) {
                        listOfCalendar.removeIf(s -> s.equals(""));

                        if (!listOfCalendar.isEmpty()) {
                            for (String val : listOfCalendar) {
