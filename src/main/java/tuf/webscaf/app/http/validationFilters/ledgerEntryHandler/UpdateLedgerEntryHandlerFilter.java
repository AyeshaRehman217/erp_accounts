package tuf.webscaf.app.http.validationFilters.ledgerEntryHandler;
...
public class UpdateLedgerEntryHandlerFilter implements HandlerFilterFunction<ServerResponse, ServerResponse> {
    ...
    public Mono<ServerResponse> filter(ServerRequest request, HandlerFunction<ServerResponse> next) {
        ...
        return request.formData()
                .flatMap(value -> {
                    ...
                    if (value.getFirst("costCenterId").isEmpty()) {
                        messages.add(
                                new AppResponseMessage(
                                        AppResponse.Response.ERROR,
                                        "Cost Center Id Field Required"
                                )
                        );
                    }

                    if (!value.getFirst("costCenterId").matches("^[0-9]*$")) {
                        messages.add(
                                new AppResponseMessage(
                                        AppResponse.Response.ERROR,
                                        "Cost Center Id will contain only Numeric Values"
                                )
                        );
                    }

                    if (value.getFirst("profitCenterId").isEmpty()) {
                        messages.add(
                                new AppResponseMessage(
                                        AppResponse.Response.ERROR,
                                        "Profit Center Id Field Required"
                                )
                        );
                    }

                    if (!value.getFirst("profitCenterId").matches("^[0-9]*$")) {
                        messages.add(
                                new AppResponseMessage(
                                        AppResponse.Response.ERROR,
                                        "Profit Center Id will contain only Numeric Values"
                                )
                        );
                    }
