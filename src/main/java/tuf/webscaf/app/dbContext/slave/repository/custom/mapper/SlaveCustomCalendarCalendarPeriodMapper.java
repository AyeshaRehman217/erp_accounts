...
public class SlaveCustomCalendarCalendarPeriodMapper implements BiFunction<Row, Object, SlaveCalendarCalendarPeriodDto> {
    @Override
    public SlaveCalendarCalendarPeriodDto apply(Row source, Object o) {

        return SlaveCalendarCalendarPeriodDto.builder()
                .id(source.get("id", Long.class))
                .version(source.get("version", Long.class))
                .uuid(source.get("uuid", UUID.class))
                .calendarPeriodUUID(source.get("calendarPeriodUUID", UUID.class))
                .name(source.get("name", String.class))
                .description(source.get("description", String.class))
                .status(source.get("status", Boolean.class))
                .fiscalYear(source.get("fiscal_year", LocalDateTime.class))
                .startDate(source.get("calendar_from", LocalDateTime.class))
                .endDate(source.get("calendar_to", LocalDateTime.class))
                .calendarTypeUUID(source.get("calendar_type_uuid", UUID.class))
                .quarter(source.get("quarter", Integer.class))
                .periodNo(source.get("period_no", Integer.class))
                .isOpen(source.get("is_open", Boolean.class))
                .adjustment(source.get("adjustments", Boolean.class))
